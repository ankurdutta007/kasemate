...
/**
 * Local fallback for Vercel API routes so /api/chat works in npm run dev.
 */
function vercelApiFallback(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        if (req.method === 'POST' && (req.url === '/api/chat' || req.url === '/api/hint' || req.url === '/api/grade' || req.url === '/api/tts')) {
          const endpoint = req.url === '/api/chat' ? '/api/chat.ts' : req.url === '/api/hint' ? '/api/hint.ts' : req.url === '/api/grade' ? '/api/grade.ts' : '/api/tts.ts'
          let body = ''
          ...
          req.on('end', async () => {
            try {
              ...
              // Inject process.env into req so it survives Vite's SSR sandbox isolation
              ;(req as any).env = process.env
              
              const module = await server.ssrLoadModule(endpoint)
