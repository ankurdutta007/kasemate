...
(async () => {
  ...
  const stepSize = 100;
  const targetEnd = turnTop + runwayLength; // End is when progress hits 1.0
  ...
  while (currentScroll < targetEnd) {
    ...
    // Evaluate if image is visible by looking at CSS variable!
    const { isVisible, opacityVal } = await page.evaluate(() => {
      const section = document.getElementById('the-turn');
      const sticky = section.querySelector('div[style*="position: sticky"]');
      const motionDiv = sticky.children[0];
      const style = window.getComputedStyle(motionDiv);
      // We set '--turn-opacity' on the motionDiv! Wait, framer motion sets it inline.
      const opacityStr = motionDiv.style.getPropertyValue('--turn-opacity');
      const opacityVal = parseFloat(opacityStr);
      return { isVisible: opacityVal > 0.02, opacityVal }; 
    });
    ...
    // Capture screenshot every 3 steps to simulate a flipbook
    if (stepCount % 3 === 0) {
