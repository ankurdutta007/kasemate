...
(async () => {
  ...
  // Scroll to just above the interview section so it hasn't triggered yet
  await page.evaluate(() => {
  ...
  for (let i = 1; i <= 6; i++) {
    await page.screenshot({ path: `/Users/ankurdutta/Downloads/code/interview_seq_${i}.png` });
    await new Promise(r => setTimeout(r, 120)); // ~120ms gap for a 0.7s animation
  }
